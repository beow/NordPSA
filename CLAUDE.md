...
### Stability: rotational energy and grid strength (off by default; world `2040_stability`)

`stability.enabled` adds a Nordic requirement on rotational energy, `Σ_z w_z·E_k,z(t) ≥ ek_system_gws` (optional zone floors `ek_zone_floor_gws`), to a **dispatch**, and `stability.scr_min` a grid-strength floor per zone: short-circuit power of the committed synchronous machines ≥ `scr_min` × the zone's actual inverter infeed (wind, solar, battery discharge; DK exempt, `zones.yaml:stability.scr_exempt`, unless included via `stability.scr_joint`). The same per-hour constraints apply in **expansion** (hard there), where capacities are variables: synchronous condensers (`syncon.enabled`, `{zone} syncon`, p_nom = MVA, 100 €/kVA assumed, 1 % auxiliary loss as negative generation) and grid-forming batteries (`battery.endogenous`, `{zone} battery gfm`: inertia H·avail and fault current `sk_pu` 1.2·avail per MW regardless of operation, never counted as SCR load) contribute by capacity. World `2040_stability` = `2040_svk_mm` (kept in sync by a test) + endogenous batteries + syncons + E_k 150 GWs + SCR 2.0. Data (H, cosφ, X''_d, m_min per technology class, the `"Component:carrier"` mapping, `sync_weight`) is in `zones.yaml:stability`; settings in `defaults.yaml:stability`.
...
- **Soft in dispatch:** slack at `dispatch.stability_slack_penalty` (100 €/(MWs·h), ~15× the dearest real source) and `dispatch.stability_scr_slack_penalty` (10 €/(MVA·h) ≈ 15 €/MWh curtailed VRE at SCR 1.5). ⚠️ For SCR the penalty **steers the result**: the only real remedies are committing more synchronous plant and curtailing VRE, so the penalty decides how much VRE is curtailed before slack is used. Low penalty ⇒ slack measures the investment need; high ⇒ the operational-only cost. Slack used in > 1 % of hours means the requirement, not the penalty, is the problem. `stability_feasibility_report` prints E_k,max vs the requirement before solving.
...
- Not modelled: AC-neighbour contributions to short-circuit power, a minimum GFM share, battery state-of-charge headroom for GFM support.
...
**SCR floor 1.5** (`run470` penalty 10, `run471` penalty 100, vs `run468`): 1.5 is the level today's world (run458) clears in ≥ 99.9 % of hours in every zone; per-zone "no weaker than today" was rejected because today's value measures each zone's surplus (NO-S has almost no wind today), not its need. The floor binds all year, not only in summer: FI ~2 150 h/yr, SE-S ~990, SE-N ~590, NO-S ~70, NO-N 0. SE-N meets it by committing hydro; **FI and SE-S by curtailing VRE** — ~1.0 TWh/yr each at penalty 100 (≈ 1.2 % of FI's, 1.5 % of SE-S's VRE), with no slack. At penalty 10 curtailment is 0.8/0.7 TWh/yr and the remainder is slack (FI 160 h/yr up to 9.4 GVA, SE-S 106 h/yr up to 16.6 GVA) — the synchronous-condenser need. Implicit grid-strength fee on VRE output: FI 2.9–3.3, SE-S 1.7–2.8, SE-N 0.35 €/MWh. Mean prices rise 0.4–0.8 €/MWh in all zones. **GFM sensitivity** (`stability.tech.ibr_<wind|solar|battery>.ibr_w` = 1 − grid-forming share; GFM infeed needs no external strength): the result hinges on it. Nordic curtailment 1.94 TWh/yr all-GFL (run471) → 0.92 with 30 % of wind GFM (run472) → 0.63 with 60 % (run473) → 0.30 with 60 % wind + 30 % solar (run474). FI is wind-driven and is solved by 30–60 % GFM wind (1.00 → 0.09 → 0 TWh); SE-S binding hours are 77 % solar, so only GFM solar helps (0.95 → 0.72 → 0.39). Batteries do not matter as load: they charge in the binding hours. ⚠️ Zone aggregate (optimistic: all machines at one node; pessimistic: no AC neighbours) — compare scenarios, don't read the levels as engineering values.

**Expansion, 2024 @ 3h** (`2040_stability`; run475 baseline with free batteries, run476/477 endogenous batteries without/with requirements, run479/478 the same at half battery cost with GFM only, `battery.cost_scale=0.5`, `battery.gfm_extra_eur_per_kw=0`):
- At the `svk_2040` battery cost (1 221 €/kW ≈ 305 €/kWh for 4 h, 156 k€/MW/yr) **no batteries are built at all**, GFL or GFM — the scenario's 12 GW of free batteries are not economic in this model (solar −4.5 GW when they go). At half cost 2.6 GW are built for arbitrage alone (DK 1.85, FI 0.71).
- The requirements cost little: +50 M€/yr at full battery cost (0.92 GVA syncon in FI, +0.5 TWh curtailment), +30 M€/yr at half cost (0.55 GVA syncon + 0.23 GW extra GFM battery in FI, +0.4 TWh). SCR binds FI ~1 500 h, SE-N ~500 h, SE-S ~400 h; E_k 150 binds 50–90 h and is met by hydro commitment.
- Per MVA of short-circuit power a syncon costs ~2 300 €/yr capital + ~1 200 €/yr auxiliary loss at 100 €/kVA; a GFM add-on to a battery built anyway ~4 800 €/yr at 40 €/kW. **GFM only wins when batteries are built for arbitrage anyway** and the add-on is cheap; the syncon cost (100 €/kVA) is itself uncertain (≈ 100–250 in projects) and flips the choice at ~150–200.
**Full scale** (2023–2025 @ 3h, half battery cost, GFM only; run480 with E_k 150 + SCR 2.0 vs run481 without; 1h dispatches run482 vs run483):
- **Built for stability:** synchronous condensers **3.0 GVA in FI** and **0.56 GVA in SE-S**, GFM batteries +0.27 GW in FI (1.0 GW total; DK 3.0 GW is arbitrage). Solar moves out of SE-S (−5.5 GW; +0.9 FI, +0.9 SE-N), offshore wind +0.8 GW in NO-S. Curtailment +0.6 TWh/yr. Objective +0.14 G€/yr (+0.45 %; the objective contains non-cost terms, so read it as indicative). In expansion E_k binds 535 h/yr, SCR FI 1 140, SE-N 870, SE-S 730, NO-S 180 h/yr.
- **1h dispatch** (frozen fleet, soft): E_k never needs slack; SCR slack in SE-S 84 h/yr (up to 13.8 GVA) and FI 15 h/yr (3.8 GVA) — below the 1 % alarm, but the 3h expansion under-sizes SE-S's midday solar peaks. Mean prices SE-N/SE-S +1.8–1.9 €/MWh, FI +0.7, NO-S −0.6; VRE output −1.6 TWh/yr; reservoir end stock −0.2 TWh.
- Runtime on the current machine: 14.9 h (with requirements) and 8.0 h (without) for the expansions, ~1 h per 1h dispatch.
- Solve time: the new extendable components, not the stability constraints, dominate — 25 min baseline, 89 min with GFL+GFM batteries and syncons, 152 min with requirements; GFM only: 81 min.

**Joint SCR requirement SE-S + DK2** (`stability.scr_joint`, off by default; tested with `{SE-S: {DK: 0.35}}`): a share of an exempt zone's short-circuit power *and* inverter infeed is added to a host zone's floor, `S_k,SE-S + 0.35·S_k,DK ≥ scr_min·(P_IBR,SE-S + 0.35·P_IBR,DK)`. DK2 (≈ 0.35 of DK) is AC-connected to SE4; a constant stiffness credit to DK2 would count SE4's machines twice. Only exempt zones may be added and a zone's shares sum to ≤ 1 (`constraints/stability.py:scr_joint`); the constraint keeps the name `scr-SE-S`. DK's own SCR on its own machines is median 0.9 (CHP only, ≤ 7 GVA vs 32 GW solar), which is why DK stays exempt on its own.
- **Frozen fleet** (`run485` vs `run484`, senuc15exo 1h dispatch, E_k 120, SCR 2.0, penalty 100): Nordic curtailment 9.5 → 16.1 TWh/yr (SE-S 1.9 → 5.7, DK 1.6 → 4.6), all of the increase at midday Apr–Sep, solar-driven (≈ 4 TWh solar, 2.6 wind — they bid the same, so the split is arbitrary); SE-S floor binds 1 067 → 1 664 h/yr; prices +1.5–2.5 €/MWh; continental export −4.2 TWh. ⚠️ DK curtailment is overstated: 1 MW curtailed anywhere in DK relieves only 0.35 MW, so ~2/3 of it corresponds to DK1 plants that would not help DK2.
- **Expansion** (2024 and 2025 @ 3h, one year each, run486/487 and run488/489 with/without): +2.3 / +3.3 GVA synchronous condensers in SE-S (none in DK — they would count only 0.35), SE-S solar −1.8 GW in 2024, curtailment only +0.4 / +0.6 TWh, objective +44–46 M€/yr (+0.17 %). With investment DK2's load is cheap; run485's curtailment is a frozen-fleet effect. One-year runs take ~2.3 h each vs 15 h for three years.
- ⚠️ SE-S is SE3+SE4: Ringhals/Forsmark/Oskarshamn count as fully available to Skåne and DK2, so the "one point" optimism is large exactly here.

**Inertia vs Svk LMA26** (docs/lma2026.pdf §5.8, pp. 98–103): Svk counts hours < 120 GWs in *normal operation* (no contingency subtracted; N-1 is implicit in the 120 threshold). run461 corresponds to Svk's MM 2040 (1.5 GW new nuclear); Svk reports 7 h/yr, run484's requirement binds ~9 h/yr. Without a requirement our count is a range, because commitment is undetermined: run461 1 148 (u = p) to 278 h/yr (u = p/m_min). The median matches Svk's (~205–210 GWs); our summer-night bottom is deeper (87 GWs at best vs ~110–115).
