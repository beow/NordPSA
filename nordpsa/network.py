...
def hydro_water_value_curve(cfg: dict):
    """Returnerar en extra_functionality-callback som ger reservoarernas lagrade
    energi ett SOC-beroende marginalvärde V(SOC) → säsongsvarierande vattenvärde.

    Mekanism: energikapaciteten delas i N tranches; hjälpvariabler e_i,t ∈ [0, w]
    med Σ_i e_i,t = SOC_t och objektivterm −Σ_t dt·Σ_i v_i·e_i,t där v_i avtar
    konvext med fyllnaden (botten värdefullast). Lagringsbalansens dual teleskoperar
    då: μ_t = μ_{t+1} + dt·v(SOC_t) — vattenvärdet FALLER framåt i tiden när SOC är
    låg (v>0, vinter/vår) och STIGER när SOC är hög (v<0, sommar/höst) → max tidig
    vinter, min vid vårflod, i stället för platt.

    Kurvan är NOLL-CENTRERAD kring neutral_frac (positiv under, negativ över).
    Det krävs för cyklisk SOC: dualrekursionen runt cykeln tvingar Σ_t v(SOC_t) ≈ 0,
    så en helt positiv kurva skulle slå SOC i taket (gratisbelöning för full
    reservoar). Cyklisk SOC + soc_initial-pinnen BEHÅLLS — de ankrar nivå och
    energibudget; kurvan omfördelar bara vattenvärdet säsongsvis.

    Config (zones.yaml, hydro_water_value): n_tranches, v_empty_eur_mwh,
    v_full_eur_mwh, shape (konvexitet), neutral_frac (nollkorsning),
    ramp_hours (EUR/MWh-ankarna fördelas över denna hold-tid → per-timme-värden),
    scale (snabb amplitudskalning, CLI --wv-scale). Per zon-override under zones:.
    """
    wcfg = cfg.get("hydro_water_value", {})

    def _curve(zone: str, n_tranches: int) -> np.ndarray:
        zc = {**wcfg, **wcfg.get("zones", {}).get(zone, {})}
        v_empty = float(zc.get("v_empty_eur_mwh", 60.0))
        v_full  = float(zc.get("v_full_eur_mwh", 0.0))
        shape   = float(zc.get("shape", 1.5))
        neutral = float(zc.get("neutral_frac", 0.6))
        ramp_h  = float(zc.get("ramp_hours", 4380.0))
        scale   = float(zc.get("scale", 1.0))

        def V(m):  # EUR/MWh-ankare, konvext avtagande i fyllnad m
            return v_full + (v_empty - v_full) * (1.0 - m) ** shape

        mids = (np.arange(n_tranches) + 0.5) / n_tranches
        return scale * (V(mids) - V(neutral)) / ramp_h   # EUR/MWh per timme, centrerad

    def _extra_functionality(n: pypsa.Network, snapshots: pd.DatetimeIndex) -> None:
        m = n.model
        soc = m.variables["StorageUnit-state_of_charge"]
        w = n.snapshot_weightings.objective.loc[snapshots]
        n_tr = int(wcfg.get("n_tranches", 5))
        tranche_idx = pd.Index(range(n_tr), name="wv_tranche")

        terms = []
        for su_name, su in n.storage_units.iterrows():
            if su.carrier != "hydro":
                continue
            e_max = float(su.p_nom * su.max_hours)
            if e_max <= 0:
                continue
            vc = _curve(su.bus, n_tr)
            e = m.add_variables(
                lower=0.0, upper=e_max / n_tr,
                coords=[snapshots, tranche_idx],
                name=f"hydro_wv_e-{su_name}",
            )
            m.add_constraints(
                e.sum("wv_tranche") == soc.sel(name=su_name),
                name=f"hydro_wv_link-{su_name}",
            )
            coef = xr.DataArray(
                np.outer(w.values, vc),
                coords={"snapshot": snapshots, "wv_tranche": tranche_idx},
                dims=("snapshot", "wv_tranche"),
            )
            terms.append((coef * e).sum())
            print(f"  → V(SOC)-kurva {su.bus}: {n_tr} tranches, "
                  f"v_c [{vc[0]*4380:+.1f} … {vc[-1]*4380:+.1f}] EUR/MWh per 4380h")

        if not terms:
            print("  Varning: --water-value-curve — inga hydro-reservoarer hittades")
            return
        reward = terms[0]
        for t in terms[1:]:
            reward = reward + t
        m.objective = m.objective.expression - reward
