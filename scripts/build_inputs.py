...
Utdata (data/processed/), ett steg per fil:
  load      load.parquet             — lastprofil per zon (MW, timvis)
  vre       vre_profiles.parquet     — p_max_pu för vind och sol per zon
            vre_pnom.yaml            — estimerade installerade effekter för vind/sol
  nuclear   nuclear_profile.parquet  — kärnkraftsprofil p_max_pu per zon
  thermal   thermal_profile.parquet  — must-run termisk (MW) per zon
  prices    market_prices.parquet    — kontinent- och zonpriser
  ev        ev_profiles.parquet      — elbilsladdningsprofiler

Inflödesparametrarna (config/hydro_params.yaml) byggs INTE här; de är handkalibrerade.
Värmelasten byggs av scripts/build_heat.py.

Användning:
    python scripts/build_inputs.py                          # alla steg till data/processed/
    python scripts/build_inputs.py --only prices ev         # bara vissa steg
    python scripts/build_inputs.py --out-dir /tmp/proc      # provbygge utan att röra data/processed/
"""
import argparse
import sys
...
from nordpsa.data.esett import NORDPSA_ZONES

from nordpsa.settings import ROOT

RAW_DIR       = ROOT / "data" / "raw"
PROCESSED_DIR = ROOT / "data" / "processed"
CONFIG_PATH   = ROOT / "config" / "zones.yaml"
...
def build_load(out_dir: Path = PROCESSED_DIR) -> pd.DataFrame:
    """
    ...
    out.index.name = "time"
    out.to_parquet(out_dir / "load.parquet")
...
def build_vre_profiles(out_dir: Path = PROCESSED_DIR) -> pd.DataFrame:
    """
    ...
    out = _trim(pd.DataFrame(result))
    out.index.name = "time"
    out.to_parquet(out_dir / "vre_profiles.parquet")
    print(f"  → vre_profiles.parquet")

    with open(out_dir / "vre_pnom.yaml", "w") as f:
...
def build_nuclear_profile(cfg: dict, out_dir: Path = PROCESSED_DIR) -> pd.DataFrame:
    """
    ...
    out.index.name = "time"
    out.to_parquet(out_dir / "nuclear_profile.parquet")
...
def build_thermal_profile(out_dir: Path = PROCESSED_DIR) -> pd.DataFrame:
    """
    ...
    Sparas som absoluta MW; nordpsa/network/generation.py beräknar p_nom = max per zon.
    """
    ...
    out = pd.DataFrame(result)
    out.index.name = "time"
    out.to_parquet(out_dir / "thermal_profile.parquet")
...
# Se nordpsa/profiles/hydro_inflow.py för modellbeskrivning.
...
def build_market_prices(out_dir: Path = PROCESSED_DIR) -> pd.DataFrame:
    """
    ...
    out = pd.DataFrame(result)
    out.index.name = "time"
    out.to_parquet(out_dir / "market_prices.parquet")
...
def build_ev_profiles(out_dir: Path = PROCESSED_DIR) -> pd.DataFrame:
    ...
    ladda-direkt-form, _bridge_inflex). Skalas/appliceras per zon i nordpsa/network/ev.py (add_ev).
    ...
    out.index.name = "time"
    out.to_parquet(out_dir / "ev_profiles.parquet")
...
STEPS = {
    "load":    lambda cfg, out: build_load(out),
    "vre":     lambda cfg, out: build_vre_profiles(out),
    "nuclear": lambda cfg, out: build_nuclear_profile(cfg, out),
    "thermal": lambda cfg, out: build_thermal_profile(out),
    "prices":  lambda cfg, out: build_market_prices(out),
    "ev":      lambda cfg, out: build_ev_profiles(out),
}
...
def main() -> None:
    ...
    ap.add_argument("--only", nargs="+", choices=list(STEPS), metavar="STEG",
                    help=f"bygg bara dessa steg ({', '.join(STEPS)}); default alla")
    ap.add_argument("--out-dir", type=Path, default=PROCESSED_DIR, metavar="MAPP",
                    help=f"var filerna skrivs (default {PROCESSED_DIR.relative_to(ROOT)})")
    args = ap.parse_args()

    args.out_dir.mkdir(parents=True, exist_ok=True)
    cfg = load_config()
    for name in (args.only or list(STEPS)):
        STEPS[name](cfg, args.out_dir)
    print(f"\nKlart! Indata sparade i {args.out_dir}/")
